...
//////////////////////////////////////////////////////////////////////////////////////////////  
	...
	var questions = [];
	questions[0] = new question("下列哪個是「彰化」的名產？",["肉圓、綠豆糕","貢丸、柿餅","太陽餅、鳳梨酥","奶油酥餅、蛋黃酥"],"1"); 
	questions[1] = new question("下列哪個是「台中」的名產？",["太陽餅","大溪豆干","鐵蛋","麥芽酥"],"1"); 
	questions[2] = new question("下列哪個是「甲仙」的名產？",["芋頭酥","麵線","一條根","牛肉乾"],"1"); 
	questions[3] = new question("逢甲夜市位於哪一個縣市？",["台中市","台北市","高雄市","南投縣"],"1"); 
	questions[4] = new question("「金針花海」是台灣哪個地方的景點？",["花東縱谷","太魯閣","陽明山","太平山"],"1");	
	questions[5] = new question("下列哪個是高雄的名產？",["旗鼓餅、木棉酥","蠶絲、牛心柿","埔里紹興酒、米粉"," 麻豆文旦、玉井芒果"],"1"); 
	questions[6] = new question("下列哪個是宜蘭的名產？",["鴨賞、牛舌餅","唱片餅、麻糬","檳榔心、釋迦","鹹餅、花生糖"],"1"); 
	questions[7] = new question("下列哪個是新竹的名產？",["米粉、貢丸","冰沙餅、鹹蛋糕","紅仁鴨蛋、烏魚子","富里米、金針花"],"1"); 
	questions[8] = new question("下列哪個是苗栗的名產？",["大湖草莓、金桔","米苔目、洛神花","貢糖、菜刀","糯米醋、	地瓜酥"],"1"); 
	questions[9] = new question("下列哪個是豐原的名產？",["冰沙餅、鹹蛋糕","埔里紹興酒、茭白筍","凸餅、新港飴","度小月擔仔麵、土魠魚羹"],"1");	
	
	var content;     //present question 
	var user,enemy;
	
	var index                   // number of question
...
////////////////////////////////////////////
        //generate  questions ask to user
 		function getQuestion(){                            
			...
			var divtext = document.createTextNode(questions[index].ques);   
			...
			for(var i = 0;i < 4;i++){                    //create  radios to display question's options
				...
				textnode = document.createTextNode((i+1) +":" + questions[index].options[i]);
 		...
 		function answerQuestion(){
			...
			for(var i=0; i < form.ask.length; i++){
		    	if (form.ask[i].checked){ 
        	    	...
        	    	audio.loop = false;
	        	    if(ans != questions[index].ans){     //check user answer is correct or not
